...
#' volumstat4 <- t03895(region_level = "fylke", periode = c("2021"))

t03895 <- function(region_level = "fylker2020", periode = TRUE ){ # 1996 - dd
  ...
  pxdt <- PxWebApiData::ApiData12("http://data.ssb.no/api/v0/en/table/03895",
    ...
    dplyr::mutate(
      ...
      TRUE ~ "annet"
    )) %>% select(region, assortment, Tid, Region, Sortimentkode, m3 = value, species, assortm.grp)
  ...
  pxdt_no <- PxWebApiData::ApiData12("http://data.ssb.no/api/v0/no/table/03895",
    ...
    dplyr::mutate(
      ...
      sortimentgruppe = dplyr::case_when(
        ...
        TRUE ~ "annet"
      )) %>% select(region, sortiment, Tid, Region, Sortimentkode, m3 = value, treslag, sortimentgruppe)

 pxdt_all <- pxdt %>% left_join(pxdt_no, by = c("region", "Region", "Tid", "Sortimentkode", "m3"))
