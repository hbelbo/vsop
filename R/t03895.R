...
t03895 <- function(region_level = "fylke"){ # 1996 - dd
  ...
  if ( !(tolower(region_level) %in% c("fylke", "kommune"))) { stop("warning: to get result, region_level should be one of 'fylker', 'kommune'" )}
...
# PxWebApiData::ApiData("http://data.ssb.no/api/v0/no/table/03895", returnMetaValues = TRUE)
  ...
  pxdt <- PxWebApiData::ApiData12("http://data.ssb.no/api/v0/en/table/03895",
                                Region =  list(aggf, reginnd),
                                Tid = T, #c("2010", "2016", "2017"),
                                Treslag = T # 10i)
                                )  %>%

    dplyr::select(-.data$NAstatus, -.data$Tid) %>%
