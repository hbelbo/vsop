...
t03794 <- function(region_level = "fylke"){
  ...
  ### This does not work until SSB has fixed the "Aggregate to 2020 Region" -----
  ...
  # if(region_level == "fylke"){
  #   reginnd <-   flk2020;
  #   #aggf <-  "agg:KommFylker" ;
  #   aggf <- "agg_single:FylkerGjeldende"
  # } else {
  #   reginnd <- kmn2020;
  #   aggf <- "agg:KommSummer";
  # }
#
#   pxdt <- PxWebApiData::ApiData12("http://data.ssb.no/api/v0/no/table/03794",
#                                   Region =  list(aggf, reginnd),
#                                   Tid = TRUE )
   metadt <- PxWebApiData::ApiData("http://data.ssb.no/api/v0/no/table/03794", returnMetaData = TRUE)
  #metadf <- PxWebApiData::ApiData("http://data.ssb.no/api/v0/no/table/03794", returnMetaFrames = TRUE)
  metadv <- PxWebApiData::ApiData("http://data.ssb.no/api/v0/no/table/03794",  returnMetaValues = TRUE)
